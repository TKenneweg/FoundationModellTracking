...
if __name__ == "__main__":
    ...
    if config.model_name == "BoxMlpModelConvLinear":
        model = BoxMlpModelConvLinear(c=C_EMB_SAM, w=W_EMB_SAM, h=H_EMB_SAM) 
    model.to("cuda")
    ...
    dataset  = GOT10kDatasetImg(config.foldername, train=True, n_train=config.n_train, freq=config.train_freq)
    ...
    backbone = SamPredictor(sam)
    backbone.model = backbone.model.to("cuda")
    ...
    for epoch in range(config.epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                img = backbone.transform.apply_image_torch(img)
                backbone.set_torch_image(img,(WIDTH,HEIGHT))
                emb = backbone.features
                
                
                img = backbone.transform.apply_image_torch(img)
                backbone.set_torch_image(img,(WIDTH,HEIGHT))
                objemb = backbone.features


                objemb = getPatchesInBox(objbox, objemb, W_EMB_SAM, H_EMB_SAM, path) #returns list
                ...
                for obj in objemb:
                    if obj.shape[0] == 0 or obj.shape[1] == 0 or obj.shape[2] == 0 or obj.shape is None:
                        abort = True
                        break
                    tmp.append(F.interpolate(obj.unsqueeze(0), size=(W_EMB_SAM// 4, H_EMB_SAM//4), mode="bilinear", align_corners=False)[0])
                if abort:
                    print("######### empty objemb ##########")
                    continue
                objemb = tmp
            ...
            if i % config.log_freq == 0 and i > 0:
                ...
                if i> 25000:
                    torch.save(model, f"models/{wandb.run.name}save.pt")
        ...
        with torch.no_grad():
            for i, data in enumerate(test_dataloader):
                ...
                objemb = getPatchesInBox(objbox, objemb, W_EMB_SAM, H_EMB_SAM) #returns list
                ...
                if abort:
                    ...
                    continue
                objemb = tmp  
                # objemb = [F.interpolate(obj.unsqueeze(0), size=(W_EMB_DINO// 4, H_EMB_DINO//4), mode="bilinear", align_corners=False)[0] for obj in objemb] #interpolate
                objemb = torch.stack(objemb) #stack to get batch dimension
